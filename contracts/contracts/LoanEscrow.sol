...
/// @title LoanEscrow
/// @notice Holds lender liquidity and manages the loan lifecycle.
///         Loan disbursement and repayment are routed via x402 protocol offchain;
...
contract LoanEscrow is Ownable, ReentrancyGuard {
    ...
    struct LenderTerms {
        address lender;
        uint256 availableLiquidity;   // wei
        uint256 maxLoanSize;          // wei
    ...
    constructor(address _trustScore, address _agentRegistry) Ownable(msg.sender) {
    ...
    /// @notice Lender deposits ETH liquidity into escrow.
    function deposit() external payable nonReentrant {
        ...
        lenderTerms[msg.sender].availableLiquidity += msg.value;
        lenderTerms[msg.sender].active = true;

        if (lenderTerms[msg.sender].maxLoanSize == 0) {
            lenderTerms[msg.sender].maxLoanSize = msg.value;
    ...
    /// @notice Lender withdraws available (not locked) liquidity.
    ...
    ///         Funds are transferred via x402 offchain; this records state.
    function createLoan(
        address lender,
        address borrower,
        uint256 principal,
    ...
    /// @notice Record repayment after x402 payment confirmed offchain.
