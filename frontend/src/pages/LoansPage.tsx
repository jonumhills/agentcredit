  ...
  return (
    <div className="max-w-6xl mx-auto px-5 py-8 pb-24 space-y-6">
      ...
      <div>
        ...
        <p className="text-okx-muted text-sm">All loans created via BancoProtocol on X Layer Testnet</p>
      </div>

      {/* Summary cards */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <SummaryCard label="Total Loans"   value={loans.length} />
        <SummaryCard label="Active"        value={active}   color="text-okx-blue" />
        ...
        {(["ALL","ACTIVE","REPAID","DEFAULTED"] as const).map(f => (
          <button
            ...
            className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${
              filter === f ? "bg-white text-black" : "text-okx-muted hover:text-white"
            }`}
